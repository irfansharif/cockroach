...
case "${cmd}" in
    ...
    ;;
    run)
      # XXX: I'm not sure why I'm having to set GOPATH myself here, or having to specify the path to `go`.
      gcloud compute ssh "${NAME}" --ssh-flag="-A" -- "export GOPATH=/home/irfansharif/go; export PATH=/snap/bin:\$PATH; cd ~/go/src/github.com/cockroachdb/cockroach; $@"
    ;;
    sync)
