		...
		b.Run(dbName, func(b *testing.B) {
			dbFn(b, func(b *testing.B, db *sqlutils.SQLRunner) {
				for _, tracingEnabled := range []bool{false, true} {
					...
					db.Exec(b, fmt.Sprintf("SET CLUSTER SETTING sql.txn_stats.sample_rate = 0.0"))
					db.Exec(b, fmt.Sprintf("SET CLUSTER SETTING sql.trace.debug_force_real_spans = %t", tracingEnabled))
